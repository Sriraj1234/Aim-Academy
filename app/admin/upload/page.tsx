...
const UploadPage = () => {
    ...
    const processParsedData = (data: any[]) => {
        const processed = data.map((row: any, index) => {
            ...
            if (rawAns.toLowerCase() === 'a' || rawAns.toLowerCase() === 'option a') correctAnswer = 0
                ...
                // Try to match text
                const idx = options.findIndex(opt => opt && opt.trim().toLowerCase() === rawAns.toLowerCase())
    ...
    const uploadQuestionsToDB = async () => {
        if (!confirm(`Ready to upload ${parsedQuestions.length} questions?`)) return
        ...
        try {
            ...
            setUploadStats({ total: parsedQuestions.length, success: successCount, failed: failedCount })
