...
async function uploadQuestions() {
    ...
    for (const sheetName of sheetNames) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!questionText || !correctAnsRaw || !chapter) {
                // Skip empty rows or generic titles
                continue;
            }
            ...
            const docData = {
                ...
                options: [optionA, optionB, optionC, optionD].filter(o => o !== undefined && o !== null).map(String),
                correctAnswer: String(correctAnswer),
                createdAt: admin.firestore.FieldValue.serverTimestamp(),
                source: 'excel_upload_mar_2026_chem'
            };
