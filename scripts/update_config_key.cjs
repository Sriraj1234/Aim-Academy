...
// The key provided by the user, formatted as a single line with \n literals
...
const envLine = `FIREBASE_PRIVATE_KEY="${keyPart1}"`;
...
try {
    ...
    const cleanedLines = lines.filter(line => !line.trim().startsWith('FIREBASE_PRIVATE_KEY'));

    // Add new key
    cleanedLines.push(envLine);
    cleanedLines.push(''); // ensure trailing newline
